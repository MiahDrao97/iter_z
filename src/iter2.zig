...
pub fn VTable(comptime T: type) type {
    return struct {
        ...
        /// Deinit any owned memory (most iterators do not own anything).
        /// Defaults to no-op.
        deinit_fn: *const fn (*Iter(T)) void = &noopDeinit,

        /// Default deinit: no-op
        pub fn noopDeinit(_: *Iter(T)) void {}
    };
}
...
pub fn Iter(comptime T: type) type {
    return struct {
        ...
        /// Deinit the iterator.
        /// Most of the time this is a no-op, but if the iterator owns any memory, call this to free it.
        ...
        pub const OwnedSliceIterable = struct {
            slice: []const T,
            idx: usize = 0,
            allocator: ?Allocator = null, // OPTIMIZE : Should the owned slice iterable be its own structure?
            ...
            fn implDeinit(iter: *Iter(T)) void {
                const self: *OwnedSliceIterable = @fieldParentPtr("interface", iter);
                if (self.allocator) |gpa| {
        ...
        pub fn toSortedSliceOwned(
            ...
            const s: []T = try self.enumerateToOwnedSlice(allocator);
        ...
        pub fn orderBy(
            ...
            const s: []T = try self.toSortedSliceOwned(allocator, compare_context, ordering);
        ...
        pub fn orderByStable(
            ...
            const s: []T = try self.toSortedSliceOwnedStable(allocator, compare_context, ordering);
            return ownedSlice(allocator, s, null);
        }
...
test "what does this API look like?" {
    var iter = Iter(u8).slice(&[_]u8{ 1, 2, 3 });
    var select = iter.interface.select(u16, struct {
        pub fn transform(_: @This(), byte: u8) u16 {
            return byte * 2;
        }
    }{});
    while (select.interface.next()) |_| {}
}
