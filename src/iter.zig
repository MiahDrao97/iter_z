...
pub fn Iter(comptime T: type) type {
    return struct {
        ...
        ///
        /// If `error.NoSpaceLeft` is returned, the buffer is full.
        /// User can still sort the returned buffer, but not all of the elements could fit onto it, so it remains unsorted.
